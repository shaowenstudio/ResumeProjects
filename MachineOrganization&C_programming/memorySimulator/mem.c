		...
		if (current->next != NULL){
			if (former == list_head){ //former head
				if ((current->next)->size_status & 1){ // next busy
					...
					list_head->size_status += current->size_status;
				}
				else {// next free
					list_head->size_status += current->size_status + sizeof(block_header) - 1;
				...
				list_head->size_status += current->size_status + sizeof(block_header) - 1; // update remaining list size
				current->next = (current->next)->next;
				current->size_status += ((current->next)->size_status + sizeof(block_header));
			}
				...
				list_head->size_status += current->size_status + 2*sizeof(block_header) - 1;
				...
				list_head->size_status += current->size_status - 1;
				current->size_status -= 1;
			}
		}
